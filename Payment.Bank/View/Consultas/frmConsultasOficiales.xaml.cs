...
{
    /// <summary>
    /// Interaction logic for frmConsultasOficiales.xaml
    /// </summary>
    public partial class frmConsultasOficiales : MetroWindow
    {
        Bank.Core.Manager db = new Core.Manager();
        clsOficialesBE BE = new clsOficialesBE();
        int VISTA = 1;

        public frmConsultasOficiales()
        {
            InitializeComponent();
        ...
        void btnEditar_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(BE.Documento))
                {
                    ...
                    {
                        VISTA = 2;
                        frmOficiales Search = new frmOficiales();
                        Search.OnInit(BE, 2);
                        Visibility = Visibility.Hidden;
                        Search.Closed += (obj, args) =>
                        {
                            BE = new clsOficialesBE(); dataGrid1.SelectedIndex = -1;
                            Find();
            ...
            try
            {
                if (!string.IsNullOrEmpty(BE.Documento))
                {
                    if (clsVariablesBO.Permiso.Eliminar == true)
                    {
                            ...
                            {
                                OperationResult result = db.OficialesDelete(BE.OficialID);
        ...
        private void dataGrid1_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                BE = (clsOficialesBE)dataGrid1.SelectedItem;
            }
            catch { }
        }
        ...
        public void Find()
        {
            try
            {
                var Result = db.OficialesGet(txtBuscar.Text);
