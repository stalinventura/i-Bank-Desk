...
{
    /// <summary>
    /// Interaction logic for frmConsultasPuestos.xaml
    /// </summary>
    public partial class frmConsultasPuestos : MetroWindow
    {
        Core.Manager db = new Core.Manager();
        clsPuestosBE BE = new clsPuestosBE();
        int VISTA = 1;

        public frmConsultasPuestos()
        {
            InitializeComponent();
            ...
            Loaded += frmConsultasPuestos_Loaded;
            txtBuscar.KeyUp += txtBuscar_KeyUp;
            txtBuscar.Focus();
        }

        private void frmConsultasPuestos_Loaded(object sender, RoutedEventArgs e)
        {
            Find();
        }
        ...
        void btnEditar_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (BE.PuestoID > 0)
                {
                    ...
                    {
                        VISTA = 2;
                        frmPuestos Search = new frmPuestos();
                        Search.OnInit(BE, 2);
                        Visibility = Visibility.Hidden;
                        Search.Closed += (obj, args) =>
                        {
                            BE = new clsPuestosBE(); dataGrid1.SelectedIndex = -1;
                            Find();
            ...
            try
            {
                if (BE.PuestoID > 0)
                {
                    if (clsVariablesBO.Permiso.Eliminar == true)
                    {
                            ...
                            {
                                OperationResult result = db.PuestosDelete(BE.PuestoID);
        ...
        private void dataGrid1_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                BE = (clsPuestosBE)dataGrid1.SelectedItem;
            }
            catch { }
        }
        ...
        public void Find()
        {
            try
            {
                var Result = db.PuestosGet(txtBuscar.Text);
