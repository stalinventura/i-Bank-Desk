...
{
    /// <summary>
    /// Interaction logic for frmConsultasPasarelas.xaml
    /// </summary>
    public partial class frmConsultasPasarelas : MetroWindow
    {
        Core.Manager db = new Core.Manager();
        clsPasarelasBE BE = new clsPasarelasBE();
        int VISTA = 1;

        public frmConsultasPasarelas()
        {
            InitializeComponent();
            ...
            Loaded += frmConsultasPasarelas_Loaded;
            txtBuscar.KeyUp += txtBuscar_KeyUp;
            txtBuscar.Focus();
        }

        private void frmConsultasPasarelas_Loaded(object sender, RoutedEventArgs e)
        {
            Find();
        }
        ...
        void btnEditar_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (BE.PasarelaID>0)
                {
                    ...
                    {
                        VISTA = 2;
                        frmPasarelas Search = new frmPasarelas();
                        Search.OnInit(BE, 2);
                        Visibility = Visibility.Hidden;
                        Search.Closed += (obj, args) =>
                        {
                            BE = new clsPasarelasBE(); dataGrid1.SelectedIndex = -1;
                            Find();
            ...
            try
            {
                if (BE.PasarelaID>0)
                {
                    if (clsVariablesBO.Permiso.Eliminar == true)
                    {
                            ...
                            {
                                OperationResult result = db.PasarelasDelete(BE.PasarelaID);
        ...
        private void dataGrid1_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                BE = (clsPasarelasBE)dataGrid1.SelectedItem;
            }
            catch { }
        }
        ...
        public void Find()
        {
            try
            {
                var Result = db.PasarelasGet(txtBuscar.Text);
