...
namespace Payment.Bank.View.Informes
{
    ...
    /// </summary>
    public partial class frmPrintRecibosGetByReciboID : Window
    {
        Manager db = new Manager();
        int ReciboID = 0;


        public frmPrintRecibosGetByReciboID()
        {
            InitializeComponent();
        ...
        private void frmPrintRecibosGetByReciboID_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                rptRecibosGetByReciboID info = new rptRecibosGetByReciboID();
                var bs = db.PrintRecibosGetByReciboID(ReciboID);
                if (bs != null)
                {
                   
                    info.SetDataSource(bs);
                    var setting = clsVariablesBO.UsuariosBE.Sucursales.Configuraciones.Impresiones.Where(x => x.TipoImpresionID == 100001 && x.DispositivoID == Common.FingerPrint.GetKey());
                    if (setting.Count() == 0)
                    {
                                    ...
                                    foreach (String Impresoras in PrinterSettings.InstalledPrinters)
                                    {
                                        if (Impresoras.ToString() == Impresora.Local || Impresoras.ToString() == Impresora.Red)
                                        {
                                            ...
                                            for (int i = 1; i <= _Copy; i++)
                                            {
                                                ...
                                                Print.RecibosPrint(ReciboID, Impresoras.ToString(), Impresora.Papel, Convert.ToSingle(Impresora.Ancho), Convert.ToSingle(Impresora.Alto));
                                            }
                    ...
                    string path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Correos");
                    ...
                    try
                    {
                        foreach (var item in Directory.GetFiles(path, $"{ReciboID}.pdf"))
                        {
                            File.SetAttributes(item, FileAttributes.Normal);
                            File.Delete(item);
                        }

                        string file = Path.Combine(path, $"{ReciboID}.pdf");
                        info.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, file);
                    }
                    catch { }
                }
                else
